...
public class ServiceImpl implements Service {
     ...
     *
     * Since the ending points of available data will always be included no matter the granularity, a user
     * may also choose to resend the request with the ending points defining the range if the returned batch size
     * is inadequate, so this is just a sanity check to avoid the double round trip to source data.
     */
    private static final int MAXIMUM_GRANULARITY = (int) TimeUnit.SECONDS.convert(1, TimeUnit.DAYS);
